...
class EventBasedMetrics:
    ...
    def record_prediction(self, edge_id: str, prediction_time: float, confidence: float):
        ...
        # FIX recall: road-key cooldown 90s→45s. The 90s cooldown was
        # blocking T-GNN early warnings — a prediction at t=200 blocked all
        # recordings until t=290, causing accidents at t=250 to be unmatched.
        # 45s allows multiple predictions per accident window (180s) while
        # still preventing the same edge from flooding the metrics log.
        if prediction_time - last_pred_time < 45:
...
class RealTimePredictor:
    ...
    def load_model(self):
        """Load trained student model and supporting files"""
        import glob
        
        print("\n" + "="*70)
        print("LOADING REAL-TIME PREDICTION MODEL")
        print("="*70)
        ...
        # Only sort if more than one file (single-item list has no getmtime)
        if len(model_files) > 1:
            model_files.sort(key=os.path.getmtime, reverse=True)
        model_file = model_files[0]
        
        try:
            # Load model — handle both dict format and raw model format
            ...
            if isinstance(model_data, dict):
                ...
                self.threshold = 0.35   # F2-optimal default (recall-weighted)
            ...
            # Load scaler — Priority 1: student_output (written by Phase 4 v2)
            ...
            # Load feature columns — Priority 1: new pipeline features (28 cols)
            if os.path.exists('student_output/feature_columns.json'):
                ...
                if feature_files:
                    ...
                    # Fallback — includes temporal features
                    self.feature_cols = [
            ...
            # ── Phase 13 T-GNN: load student + adjacency ─────────────────
            try:
                import glob as _g2
                import json as _j2

                _dep = sorted(_g2.glob('tgnn_output/tgnn_deployment_*.json'),
                              key=os.path.getmtime)
                if _dep:
                    with open(_dep[-1]) as _fh:
                        _tcfg = _j2.load(_fh)
                    _sm = _tcfg.get('student_path', '')
                    _ap = _tcfg.get('adjacency_path', '')
                    _thr = float(_tcfg.get('threshold', 0.301))
                    _nw  = float(_tcfg.get('neighbor_weight', 0.35))

                    if _sm and os.path.exists(_sm) and _ap and os.path.exists(_ap):
                        import tensorflow as _tf2
                        self.tgnn_model     = _tf2.keras.models.load_model(_sm)
                        with open(_ap) as _fh2:
                            _adj_data = _j2.load(_fh2)
                        # Rebuild adjacency dicts
                        self.road_adj    = {int(k): v for k, v in
                                            _adj_data['adj'].items()}
                        self.edge_to_idx = _adj_data['edge_to_idx']
                        self.idx_to_edge = {v: k for k, v in
                                            self.edge_to_idx.items()}
                        # FIX 1: use blended_threshold from deployment JSON.
                        # blended_threshold is F2-calibrated on the actual
                        # 0.70*tgnn+0.30*ml blended score Phase 5 uses at inference.
                        # The old hardcoded 0.56 was suppressing ~35% of valid predictions.
                        self.tgnn_threshold = float(_tcfg.get('blended_threshold',
                                                    _tcfg.get('threshold', 0.56)))
                        self.neighbor_w     = _nw
                        self.tgnn_mode      = True

                        # Load T-GNN scaler (27-feature StandardScaler)
                        _scalers = sorted(_g2.glob('tgnn_output/tgnn_scaler_*.pkl') +
                                          _g2.glob('student_output/scaler.pkl'),
                                          key=os.path.getmtime)
                        if _scalers:
                            self.tgnn_scaler = joblib.load(_scalers[-1])

                        print(f"✅ T-GNN Student loaded: {os.path.basename(_sm)}")
                        print(f"   Threshold={self.tgnn_threshold:.4f}  NeighborW={_nw:.2f}  (blended_threshold from JSON, was hardcoded 0.56)")
                        print(f"   Road graph: {len(self.edge_to_idx):,} nodes  "                              f"{sum(len(v) for v in self.road_adj.values()):,} edges")
                    else:
                        print("ℹ️  T-GNN model files not found — skipping T-GNN")
                else:
                    print("ℹ️  No tgnn_deployment_*.json found — T-GNN disabled")
            except Exception as _tgnn_err:
                print(f"⚠️  T-GNN load failed ({_tgnn_err}) — continuing without T-GNN")
                self.tgnn_mode = False

            print("="*70)
            mode_str = "T-GNN+ML" if self.tgnn_mode else "ML-only" 
            print(f"✅ REAL-TIME PREDICTION READY  (mode={mode_str})")
            print("="*70 + "\n")
            return True
    ...
    def predict(self, features_dict, edge_id, timestamp):
        """
        T-GNN + ML prediction ensemble.
        ML branch:   tabular 27-feature RandomForest student (Phase 4).
        T-GNN branch: 1-hop graph neural network student (Phase 13).
        Ensemble:    0.65*tgnn + 0.35*ml  (T-GNN active) or ML-only fallback.
        Returns dict: probability, high_risk, confidence, ml_prob, mode.
        """
        ...
        try:
            ...
            # ── Store live ML score for graph propagation (T-GNN) ──────────
            self._live_ml_scores[edge_id] = ml_prob

            # ── T-GNN branch (Phase 13 Temporal GNN) ────────────────────────
            tgnn_prob = None
            if self.tgnn_mode and self.tgnn_model and self.tgnn_scaler:
                # FIX recall: gate lowered 0.40→0.30. The 0.40 floor suppressed T-GNN
                # on genuinely risky edges where tabular ML was uncertain (sparse
                # training data) — exactly the cases T-GNN fixes via graph propagation.
                if ml_prob < 0.10:  # FIX 2: gate lowered 0.30→0.10
                    tgnn_prob = None  # skip T-GNN, fall through to ML-only
                else:
                    try:
                        nidx = self.edge_to_idx.get(edge_id, -1)
                        if nidx >= 0:
                            neighbors = self.road_adj.get(nidx, [])
                            if neighbors:
                                # Only blend with neighbours that show real danger (≥0.15).
                                neigh_scores = [
                                    self._live_ml_scores.get(
                                        self.idx_to_edge.get(n, ''), ml_prob)
                                    for n in neighbors
                                ]
                                active_neigh = [s for s in neigh_scores if s >= 0.15]
                                neigh_avg = (float(sum(active_neigh) / len(active_neigh))
                                             if active_neigh else ml_prob)
                            else:
                                neigh_avg = ml_prob
                            graph_score = ((1 - self.neighbor_w) * ml_prob
                                           + self.neighbor_w * neigh_avg)
                        else:
                            graph_score = ml_prob
                        # Build 29-feature input [27 scaled + ml_score + graph_score]
                        feat_27 = [float(features_dict.get(c) or 0)
                                   for c in self.feature_cols[:27]]
                        X_27 = self.tgnn_scaler.transform([feat_27])
                        X_29 = np.concatenate(
                            [X_27, [[ml_prob, graph_score]]], axis=1
                        ).astype(np.float32)
                        tgnn_prob = float(
                            self.tgnn_model.predict(X_29, verbose=0)[0, 0])
                    except Exception:
                        tgnn_prob = None

            # ── Ensemble combination ──────────────────────────────────────────
            # FIX: use the BLENDED prob against a single threshold for is_high_risk.
            # The old OR rule (tgnn_prob >= thr OR ml_prob >= thr) caused either
            # model to independently fire an alert — compounding false positives.
            # A single blended score decision is more conservative and calibrated.
            # FIX recall: T-GNN weight 0.65→0.70. T-GNN captures graph-propagated
            # risk that tabular ML misses. Strong T-GNN (0.80) + weak ML (0.35)
            # previously gave 0.65*0.80+0.35*0.35=0.64 — barely above threshold,
            # missing many genuine graph-based warnings.
            if tgnn_prob is not None:
            ...
            prob         = float(prob)
            # Use the model-appropriate threshold:
            #   T-GNN active → use tgnn_threshold (F2-calibrated by Phase 13, e.g. 0.282)
            #   ML-only      → use self.threshold  (F2-calibrated by Phase 4,  e.g. 0.566)
            # Using the ML threshold when T-GNN is active suppresses ~40% of correct
            # high-risk alerts because T-GNN outputs are calibrated to a lower range.
            _active_threshold = (self.tgnn_threshold
                                 if (tgnn_prob is not None and self.tgnn_mode)
                                 else self.threshold)
            is_high_risk = prob >= _active_threshold
                    ...
                    if state.predictor and state.predictor.model:
                        ...
                        if prediction:
                            ...
                            if prediction['high_risk']:
                                ...
                                # FIX recall: threshold 0.60→0.50, cooldown 60s→30s
                                # Original 0.60 silently discarded genuine 0.50-0.59 alerts.
                                # 60s cooldown blocked re-recording on persistent high-risk
                                # edges where the accident was still 90s away.
                                # FIX: lower threshold 0.50→0.33 to match blended_threshold
                                # and reduce cooldown 30s→15s so T-GNN early warnings
                                # (often 60-120s before accident) are actually recorded.
                                _metrics_thr = 0.33
    ...
    <div class="panel event-metrics-panel">
        ...
        <div style="margin-top: 20px; background-color: rgba(255,255,255,0.1); padding: 15px; border-radius: 5px;">
            ...
            <div style="font-size: 12px; opacity: 0.9;">
                ⚡ <strong>Event-Based Metrics</strong>: Each accident is one event. Prediction correct if within 120s before accident.
            </div>
        </div>
    </div>
